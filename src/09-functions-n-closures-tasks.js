...
function retry(func, attempts) {
  ...
  return () => {
    ...
    } while (attemptCount < attempts && hasError);
    return result;
  };
}
...
function logger(func, logFunc) {
  return (...args) => {
    ...
    const argumentList = args.map((val) => JSON.stringify(val)).join(',');
    logFunc(`${functionName}(${argumentList}) starts`);
    const result = func(...args);
    logFunc(`${functionName}(${argumentList}) ends`);
    return result;
  };
}
