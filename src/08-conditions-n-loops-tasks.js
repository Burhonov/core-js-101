...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const charIndex = history.findIndex((subArr) => subArr[0] === str[i]);
    if (charIndex > -1) {
      history[charIndex][1] += 1;
    } else {
      history.push([str[i], 1]);
    }
  }
  const firstSingleCharIndex = history.findIndex((subArr) => subArr[1] === 1);
  if (firstSingleCharIndex > -1) {
    return history[firstSingleCharIndex][0];
  }
  return null;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  const leftBoundary = isStartIncluded ? '[' : '(';
  const rightBoundary = isEndIncluded ? ']' : ')';
  return `${leftBoundary}${smallerNumber}, ${greaterNumber}${rightBoundary}`;
}
...
function isCreditCardNumber(ccn) {
  const init = ccn.toString();
  let sum = 0;
  let isSecondDigit = false;
  for (let i = init.length - 1; i >= 0; i -= 1) {
    const lastDigit = parseInt(init[i], 10);
    if (isSecondDigit) {
...
function getDigitalRoot(num) {
  ...
  for (let i = 0; i < stringified.length; i += 1) {
    digitSum += parseInt(stringified[i], 10);
  }
  if (digitSum < 10) {
    return digitSum;
  }
...
function isBracketsBalanced(str) {
  ...
  const history = [];
  for (let i = 0; i < str.length; i += 1) {
    if (map[str[i]] && map[str[i]] === history[history.length - 1]) {
      history.pop();
    } else {
      history.push(str[i]);
    }
...
function toNaryString(num, n) {
  let init = num;
  let result = '';
  const highestPower = Math.floor(Math.log(num) / Math.log(n));
  for (let i = highestPower; i >= 0; i -= 1) {
    result += Math.floor(init / n ** i);
    init %= (n ** i);
  }
  return result;
}
...
function getCommonDirectoryPath(pathes) {
  const result = pathes[0].split('').reduce((acc, char) => {
    const currentSubStr = `${acc}${char}`;
    if (pathes.slice(1).every((path) => path.startsWith(currentSubStr))) {
